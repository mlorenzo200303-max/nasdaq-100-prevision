"""
Nasdaq 100 — Generatore segnale giornaliero
Eseguito ogni giorno da GitHub Actions dopo la chiusura dei mercati USA.
Produce docs/signal.json letto dal sito web.
"""
...
vix = yf.download('^VIX',     period='5y', interval='1d', auto_adjust=True, progress=False)['Close'].squeeze()
tnx = yf.download('^TNX',     period='5y', interval='1d', auto_adjust=True, progress=False)['Close'].squeeze()
dxy = yf.download('DX-Y.NYB', period='5y', interval='1d', auto_adjust=True, progress=False)['Close'].squeeze()

vix = vix.rename('VIX')
tnx = tnx.rename('TNX')
dxy = dxy.rename('DXY')

print(f"✅ {len(df)} giorni scaricati  |  ultimo: {df.index[-1].date()}")
...
FEAT = [c for c in df_feat.columns
...
# Ultime 5 sessioni per il mini-grafico
...
print(f"\n{emoji} Segnale: {segnale}")
print(f"   Probabilità rialzo: {proba*100:.1f}%")
print(f"   Salvato in docs/signal.json")
